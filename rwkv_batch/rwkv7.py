...
class RWKV_x070(MyModule):
    ...
    @MyFunction
    def forward_seq_batch_samelen_idx(self, idxs:List[List[int]], state:List[torch.Tensor], full_output:bool=False):
        with torch.no_grad(): 
            z = self.z
    ...
    @MyFunction
    def forward_seq_batch_samelen_tensor(self, idxs:torch.Tensor, state:List[torch.Tensor], full_output:bool=False):
        with torch.no_grad(): 
            z = self.z
            x = z['emb.weight'][idxs]
    ...
    @MyFunction
    def forward_seq_batch_varlen(self, idxs:torch.Tensor, state:List[torch.Tensor], lens:torch.Tensor, full_output:bool=False):
        with torch.no_grad():
            L = idxs.size(1)
