...
class RWKV_x070(MyModule):
    def __init__(self, args, device=None):
        ...
        for k in keys:
            ...
            if 'att.g1' in k or 'att.g2' in k or 'att.a1' in k or 'att.a2' in k or 'att.w1' in k or 'att.w2' in k or 'att.v1' in k or 'att.v2' in k or 'ffn.value.weight' in k:
                z[k] = z[k].t()
            z[k] = z[k].squeeze().to(dtype=DTYPE, device=self.device)
    ...
    def generate_zero_state(self, bsz):
        ...
        if bsz >= 1:
            state[0] = torch.zeros((args.n_layer, 2, bsz, args.n_embd), dtype=DTYPE, requires_grad=False, device=self.device)
            state[1] = torch.zeros((args.n_layer, bsz, args.n_embd // args.head_size, args.head_size, args.head_size), dtype=DTYPE, requires_grad=False, device=self.device)
            state[2] = torch.zeros((bsz,), dtype=torch.int32, requires_grad=False, device=self.device)
        else:
            state[0] = torch.zeros((args.n_layer, 2, args.n_embd), dtype=DTYPE, requires_grad=False, device=self.device)
            state[1] = torch.zeros((args.n_layer, args.n_embd // args.head_size, args.head_size, args.head_size), dtype=DTYPE, requires_grad=False, device=self.device)
            state[2] = torch.zeros((), dtype=torch.int32, requires_grad=False, device=self.device)
        return state
    ...
    def forward_batch(self, tokens, state, full_output=False): # will modify state in-place
        ...
        if full_output == False:
            out = torch.empty((bsz, self.args.vocab_size), dtype=DTYPE, requires_grad=False, device=self.device)
        else:
            out = [torch.empty((0, self.args.vocab_size), dtype=DTYPE, requires_grad=False, device=self.device) for _ in range(bsz)]
        while True:
