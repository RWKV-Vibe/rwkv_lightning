...
void cuda_forward_seq_paged(int B, int T, int C, int H, F *state, int *page_table, F *r, F *w, F *k, F *v, F *a, F *b, F *y, int* elapsed_t);
void cuda_forward_one_paged(int B,        int C, int H, F *state, int *page_table, F *r, F* w, F *k, F *v, F *a, F *b, F *y, int* elapsed_t);
...
void forward_seq_paged(int64_t B, int64_t T, int64_t C, int64_t H, torch::Tensor &state, torch::Tensor &page_table, torch::Tensor &r, torch::Tensor &w, torch::Tensor &k, torch::Tensor &v, torch::Tensor &a, torch::Tensor &b, torch::Tensor &y, torch::Tensor &elapsed_t) {
    cuda_forward_seq_paged(B, T, C, H, state.data_ptr<F>(), page_table.data_ptr<int>(), r.data_ptr<F>(), w.data_ptr<F>(), k.data_ptr<F>(), v.data_ptr<F>(), a.data_ptr<F>(), b.data_ptr<F>(), y.data_ptr<F>(), elapsed_t.data_ptr<int>());
}

void forward_one_paged(int64_t B, int64_t C, int64_t H, torch::Tensor &state, torch::Tensor &page_table, torch::Tensor &r, torch::Tensor &w, torch::Tensor &k, torch::Tensor &v, torch::Tensor &a, torch::Tensor &b, torch::Tensor &y, torch::Tensor &elapsed_t) {
    cuda_forward_one_paged(B, C, H, state.data_ptr<F>(), page_table.data_ptr<int>(), r.data_ptr<F>(), w.data_ptr<F>(), k.data_ptr<F>(), v.data_ptr<F>(), a.data_ptr<F>(), b.data_ptr<F>(), y.data_ptr<F>(), elapsed_t.data_ptr<int>());
}
...
TORCH_LIBRARY(rwkv7_state_fwd_fp16, m) {
    ...
    m.def("forward_one_paged", forward_one_paged);
    m.def("forward_seq_paged", forward_seq_paged);
