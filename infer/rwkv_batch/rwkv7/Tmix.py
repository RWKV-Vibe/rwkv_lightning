...
############################################### W8A8 ###########################################
@MyStatic
def RWKV_x070_TMix_one_i8(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k,
    R_w, R_scale, K_w, K_scale, V_w, V_scale, O_w, O_scale, ln_w, ln_b, elapsed_t):
    xx = x_prev[0] - x
    x_prev[0] = x
    xr, xw, xk, xv, xa, xg = x+xx*x_r, x+xx*x_w, x+xx*x_k, x+xx*x_v, x+xx*x_a, x+xx*x_g

    r = linear_w8a8(xr, R_w, R_scale)
    w = F.linear(torch.tanh(F.linear(xw, w1)), w2, bias=w0)
    k = linear_w8a8(xk, K_w, K_scale)
    v = linear_w8a8(xv, V_w, V_scale)
    a = torch.sigmoid(F.linear(F.linear(xa, a1), a2, bias=a0))
    g = F.linear(torch.sigmoid(F.linear(xg, g1)), g2)
    kk = F.normalize((k * k_k).view(H,N), dim=-1, p=2.0).view(H*N)
    k = k * (1 + (a-1) * k_a)
    ...
    xx = RWKV7_ONE_OP(state, r, w, k, v, -kk, kka, elapsed_t)
    xx = F.group_norm(xx.view(1,H*N), num_groups=H, weight=ln_w, bias=ln_b, eps = 64e-5).view(H*N)
    xx = xx + ((r * k * r_k).view(H,N).sum(dim=-1, keepdim=True) * v.view(H,N)).view(H*N)
    return linear_w8a8((xx * g), O_w, O_scale), v_first

@MyStatic
def RWKV_x070_TMix_seq_i8(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k,
    R_w, R_scale, K_w, K_scale, V_w, V_scale, O_w, O_scale, ln_w, ln_b, elapsed_t):
    T = x.shape[0]
    xx = torch.cat((x_prev[0].unsqueeze(0), x[:-1,:])) - x
    x_prev[0] = x[-1,:]
...
@MyStatic
def RWKV_x070_TMix_seq_batch_i8(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k,
    R_w, R_scale, K_w, K_scale, V_w, V_scale, O_w, O_scale, ln_w, ln_b, elapsed_t):
    B,T,C = x.shape
    xx = torch.cat((x_prev[0].unsqueeze(1), x[:,:-1,:]), dim=1) - x
    x_prev[0] = x[:,-1,:]
