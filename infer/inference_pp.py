...
class InferenceEngine:
    ...
    def _sampling_device(self, logits=None):
        if isinstance(logits, torch.Tensor):
            return logits.device
        return getattr(self.model, "sample_device", torch.device("cuda:0"))

    def _state_restore_device(self):
        return getattr(self.model, "state_restore_device", "cuda")

    def _setup_rand_state(self, batch_size, device):
        with torch.cuda.device(device):
            return sample.setup_rand(random.randint(0, 2**63 - 1), batch_size)

    @staticmethod
    def _build_penalties(batch_size, device):
        return torch.zeros(batch_size, 65536, device=device)
    ...
    def _sample_tokens(
        ...
        sample_device = self._sampling_device(logits)
        with torch.cuda.device(sample_device):
            sample_rand_states = sample.setup_rand(random.randint(0, 2**63 - 1), batch_size)
            penalties = self._build_penalties(batch_size, sample_device)
    ...
    def _prefill_prompt_with_prefix_cache(self, prompt, prefix_cache_manager=None):
        ...
        if prefix_cache_manager is not None:
            cache_match = prefix_cache_manager.match_prefix_state(
                encoded_prompt,
                device=self._state_restore_device(),
            )
        ...
        try:
            while True:
                    ...
                    if len(task["input_token"]) == 0:
                        ...
                        if is_finished:
                            ...
                            if len(input_queue) > 0:
                                ...
                                state_pos = task["state_pos"]
                                self.model.reset_state_slot(states, state_pos)
                ...
                if accomplished_task_indices:
                    ...
                    states = self.model.remove_state_slots(states, sorted_slots)
